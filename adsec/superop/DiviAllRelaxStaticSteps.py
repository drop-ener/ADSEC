...
class DiviAllRelaxStaticSteps(Steps):
    def __init__(
        self,
        name: str,
        make_op: Type[OP],
        ...
        timeout_1: Optional[float] = 72000,
        timeout_2: Optional[float] = 72000,
    ):
        ...
        self._keys = ["bulk-relaxtion-step", "bulk-static-step", "slab-relaxtion-step","slab-static-step","adslab-relaxtion-1-step",\
                      "adslab-relaxtion-2-step","adslab-static-step"]

        self.step_keys = {}
        key = "bulk-relaxtion-step"
        ...
        key = "adslab-relaxtion-1-step"
        self.step_keys[key] = '--'.join(
            [str(self.inputs.parameters["flow_id"]), key]
        )
        
        key = "adslab-relaxtion-2-step"
        self.step_keys[key] = '--'.join(
            [str(self.inputs.parameters["flow_id"]), key]
        )
    ...
    def _build(
        ...
        bulk_relaxtion_step = Step(
            name=f'bulk-relaxtion-step',
            template = relaxtionSteps,
            artifacts={
                "input_work_path": self.inputs.artifacts["input_work_path"]},
            parameters={
                "flow_id": self.inputs.parameters["flow_id"],
                "path_to_prop": self.inputs.parameters["path_to_prop_relaxtion_bulk"],
                "prop_param": self.inputs.parameters["relaxtion_prop_param_bulk"],
                "inter_param": self.inputs.parameters["inter_param"],
               },
            key= self.step_keys["bulk-relaxtion-step"]
        )
        self.add(bulk_relaxtion_step)
        ...
        bulk_static_step = Step(
            ...
            artifacts={
                "input_work_path": bulk_relaxtion_step.outputs.artifacts["retrieve_path"]},
            parameters={
                "flow_id": self.inputs.parameters["flow_id"],
        ...
        adslab_relaxtion_1_step = Step(
            name=f'adslab-relaxtion-1-step',
            template = relaxtionSteps,
            artifacts={
            ...
            parameters={
                ...
                "prop_param": self.inputs.parameters["relaxtion_prop_param_1_adslab"],
                "inter_param": self.inputs.parameters["inter_param"],
               },
            key= self.step_keys["adslab-relaxtion-1-step"]
        )
        self.add(adslab_relaxtion_1_step)
        ...
        adslab_relaxtion_2_step = Step(
            name=f'adslab-relaxtion-2-step',
            template = relaxtionSteps,
            artifacts={
                "input_work_path": adslab_relaxtion_1_step.outputs.artifacts["retrieve_path"]},
            parameters={
                "flow_id": self.inputs.parameters["flow_id"],
                "path_to_prop": self.inputs.parameters["path_to_prop_relaxtion_adslab"],
                "prop_param": self.inputs.parameters["relaxtion_prop_param_2_adslab"],
                "inter_param": self.inputs.parameters["inter_param"],
               },
            key= self.step_keys["adslab-relaxtion-2-step"]
        )
        self.add(adslab_relaxtion_2_step)
        ...
        adslab_static_step = Step(
            ...
            template = staticSteps,
            artifacts={
                "input_work_path": adslab_relaxtion_2_step.outputs.artifacts["retrieve_path"]},
            parameters={
                "flow_id": self.inputs.parameters["flow_id"],
