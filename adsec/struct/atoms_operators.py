...
def calc_min_dist(mol_symbols,mol_pos,slab,default_min_dist): 

    neigh_number_set = []
    for i in range(1,len(mol_pos)):
        first_pos = mol_pos[i]
        first_elem = mol_symbols[i]
        first_pos = first_pos[np.newaxis, :]
        one_atoms  = Atoms(symbols=first_elem,positions=first_pos)
        #cut_val = []
        #cov_radius_first = covalent_radii[chemical_symbols.index(first_elem)]

        tmp_adslab = one_atoms + slab
        tmp_adslab.cell = slab.cell
        tmp_adslab.pbc = [True, True, True]
        #cutoff = natural_cutoffs(tmp_adslab)

        #for slab_elem in slab_symbols:
        #    cov_radius_slab_ele = covalent_radii[chemical_symbols.index(slab_elem)]
        #    cut_val.append((cov_radius_first+cov_radius_slab_ele)*1.05/2.0)
        #cut_val.append(cut_val[0])
      
        cut_val = [default_min_dist*0.5]*len(tmp_adslab)  
        #print(cut_val)
        nl = NeighborList(cutoffs=cut_val,skin=0.0,self_interaction=False, bothways=True)
        nl.update(tmp_adslab)
        indices, offsets = nl.get_neighbors(0)
           
        neigh_number_set.append(len(indices))
    #print(neigh_number_set)
    if max(neigh_number_set) > 0:
        return False
    else:
        return True
    ...
    if rotate_flag:
        ...
        if first_elem not in ["H"]:
            ...
            for j in range(len(rotate_values)):
                ...
                #print(slab_pos)
                min_dist_flag = calc_min_dist(mol_elem,rotated_coords,slab,default_min_dist)
                if  min_dist_flag:
                    #print(j,rotate_values[j])
